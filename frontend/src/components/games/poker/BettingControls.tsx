...
export function BettingControls({ toCall, minBet, maxBet, canCheck, onCheck, onCall, onBetOrRaise, onFold }: BettingControlsProps) {
    ...
    return (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {!canCheck && toCall > 0 && (
                <button onClick={onCall} disabled={toCall > maxBet}>Call {toCall}</button>
            )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <input
                    ...
                    style={{ width: 80 }}
                />
                <button onClick={() => onBetOrRaise(Math.min(amount, clampedMax))} disabled={!canBet}>
